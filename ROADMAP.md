...
- [x] **High: `Host::sleep` can permanently exhaust native threads.**  
...
- [x] **High: cancellation does not interrupt executing Wasm.**  
...
- [x] **High under an untrusted-Wasm threat model: native runner memory is not fully bounded.**  
...
### Phase 18: Fixed Proposed by Sol - Part 2

- [x] **Critical: vulnerable Wasmtime version.** [Cargo.toml](/Users/roba/Code/exs/Cargo.toml:31) selects Wasmtime 41, and [Cargo.lock](/Users/roba/Code/exs/Cargo.lock:3293) resolves `41.0.4`. That version is affected by a critical AArch64 Cranelift sandbox escape, and this development machine is AArch64. It is also affected by a guest-triggerable table-allocation panic. See the official [sandbox-escape advisory](https://github.com/bytecodealliance/wasmtime/security/advisories/GHSA-jhxm-h53p-jm7w) and [table-allocation advisory](https://github.com/bytecodealliance/wasmtime/security/advisories/GHSA-p8xm-42r7-89xg). Upgrade to a currently supported patched release, presently `47.0.3`, before treating native execution as isolated.
- [x] **High: Rust guest host calls can execute repeatedly.** In [host.rs](/Users/roba/Code/exs/crates/exs-guest/src/host.rs:362), a pending `HostCall` checks for its response and, when absent, calls `begin_host_call` again using the same ID. Futures may be polled repeatedly. With two concurrent guest futures, completion of one repolls the other and duplicates its host side effect, potentially racing multiple responses under one ID. Model the call as `NotStarted | Pending(id) | Complete`; a pending poll without a response must simply return `Poll::Pending`. Add a test with two calls completing out of order.
- [] **High: Wasm compilation is outside timeout and cancellation.** [ServerRunner::execute](/Users/roba/Code/exs/crates/exs-runner/src/lib.rs:119) starts timing, creates an engine, and performs `Module::new`; the actual deadline thread starts only afterward at line 124. Crafted Wasm can therefore consume unbounded compilation time despite `ExecutionLimits::timeout`. It also recompiles identical Wasm for every invocation. Introduce a `PreparedModule` compiled once against a runner-owned engine, and bound compilation separately, preferably in an isolated worker.
- [] **High for arbitrary Wasm: native allocation limits are incomplete.** [HostAbiState::new](/Users/roba/Code/exs/crates/exs-runner/src/host_abi.rs:78) limits each linear memory’s size but leaves table elements, table count, and memory count at Wasmtime defaults. `ServerRunner` and `exs run file.wasm` accept arbitrary modules, so a small module can request large host-side table allocations or multiple memories. Add explicit table/count limits and disable unused Wasm proposals. Consider rejecting modules without compiler-issued metadata if arbitrary third-party Wasm is not required.
- [] **High if ExS source is untrusted: compilation has no resource policy.** The public compiler accepts unlimited source, module count, graph size, tokens, or AST depth. [module_graph.rs](/Users/roba/Code/exs/crates/exs-compiler/src/module_graph.rs:56) grows the resolved graph without bounds, while expressions recursively parse unary operators and parentheses in [expressions.rs](/Users/roba/Code/exs/crates/exs-compiler/src/parser/expressions.rs:89). Deep input can overflow the Rust stack; broad input can exhaust memory before the runner sandbox exists. Add `CompilationLimits` and isolate server-side compilation from the main service process.
- [] **Medium: one native OS thread is created per execution deadline.** [deadline.rs](/Users/roba/Code/exs/crates/exs-runner/src/deadline.rs:48) spawns and later joins a thread for every execution. Enough concurrent slow executions can exhaust native threads. Reuse the existing shared timer mechanism or a shared epoch ticker, and enforce a runner-level concurrency limit.
- [] **Medium: native reserved host names are incomplete.** [registry.rs](/Users/roba/Code/exs/crates/exs-runner/src/registry.rs:334) reserves sleep and stream names, but [host_abi.rs](/Users/roba/Code/exs/crates/exs-runner/src/host_abi.rs:415) also intercepts now, elapsed, and DateTime operations. Registration under those names succeeds but is unreachable. The browser registry correctly reserves all names. Move this predicate into `exs-abi` and test both registries against one shared list.
- [] **Medium integrity risk: the committed runtime binary is not verified against its source.** The compiler embeds the binary directly in [exs-runtime/src/lib.rs](/Users/roba/Code/exs/crates/exs-runtime/src/lib.rs:30). CI rebuilds a separate target artifact at [ci.yml](/Users/roba/Code/exs/.github/workflows/ci.yml:42), but never compares it with the committed `exs-runtime.wasm`. A stale or modified binary can therefore diverge from reviewed Rust code. Make the build reproducible and fail CI when rebuilding changes the committed artifact.
- [] Create a reusable runner engine plus `PreparedModule`; this removes repeated configuration/JIT work and gives compilation a clear security boundary.
- [] Introduce one shared `LoadedProject` graph. Compilation currently parses graph sources twice, while documentation duplicates loading and cycle detection in [documentation/source.rs](/Users/roba/Code/exs/crates/exs-compiler/src/documentation/source.rs:37).
- [] Disable Wasmtime default features and enable only the core runtime and compiler features ExS uses. Current defaults include component model, GC, threads, async support, profiling, caching, and other unused attack surface.
- [] Replace runtime functions returning `&'static`/`&'static mut` values from global state, such as [runtime.rs](/Users/roba/Code/exs/crates/exs-runtime/src/runtime.rs:263), with scoped accessors. This makes aliasing and GC/reallocation safety locally enforceable.
